...
TENSORBOARD_DIR="${PWD}/dapoqwn_tblog"
SUBMITID="Math$(date +'%y%m%d%H%M')"

project_name='DAPOTst'
exp_name='DAPOQwen1d5bTst'
...
n_resp_per_prompt=4  #rollout.n # g
train_prompt_mini_bsz=$((train_traj_mini_bsz * n_resp_per_prompt)) # b * n * f * g
train_prompt_bsz=$((train_prompt_mini_bsz * 1)) # b * n * f * 1 * g
...
CKPTS_DIR=${CKPTS_DIR:-"${PWD}/ckpt/${project_name}/${exp_name}"}
TRAIN_FILE="${EVRL_PATH}/BT/data/dapo-math-17k.parquet"
TEST_FILE="${EVRL_PATH}/BT/data/aime-2024.parquet"
