...
class BtBatchRewardManager(BatchRewardManager):
    def __init__(self, tokenizer, num_examine, compute_score, reward_fn_key='data_source', **reward_kwargs):
    ...
    def verify(self, data):
        ...
        scores = self.compute_score(data_sources=data_sources,
                                    ...
                                    ground_truths=ground_truths,
                                    extra_infos=extras,
                                    data=data,  #@#MODIFIED 把整个data传进去吧，爱干嘛干嘛，也可以拿到rwd mdl的分数
                                    **self.reward_kwargs)
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        # If there is rm score, we directly return rm score. Otherwise, we compute via rm_score_fn
        # if 'rm_scores' in data.batch.keys():#@#去掉这个，我们需要多种reward，包括reward mdl的，同时会把rwd mdl的rwd进行加工
        #     if return_dict:
        #         return {"reward_tensor": data.batch['rm_scores']}
        #     else:
        #         return data.batch['rm_scores']
