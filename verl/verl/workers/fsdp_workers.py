...
import time
import torch
torch.set_default_dtype(torch.bfloat16) #@#BUGFIX for flash_attn 2 warning
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # normalize config
        if self._is_actor:
            self.config.actor.ppo_mini_batch_size *= self.config.rollout.n  #@#训练循环 这里mini batchSz乘rollout.n后，下面还要划分到每个训练进程,注意这里的ppo_mini_batch_size是针对整个集群而不是per_gpu的
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def setSampleParam(self, **kwargs):
        self.smpPramKwargs = kwargs
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            if self.smpPramKwargs is not None:#@#TODO add seed， if changeSeed=True, to vLLMRollout.generate_sequences @ verl/workers/rollout/vllm_rollout/vllm_rollout_spmd.py
                output = self.rollout.generate_sequences(prompts=prompts, **self.smpPramKwargs)
            else:
...
#@#MODIFIED add for med verifier
class BtRewardModelWorker(Worker):#@#只能继承Worker，不能继承RewardModelWorker
    def __init__(self, config):
        super().__init__()
        import torch.distributed
        if not torch.distributed.is_initialized():
    ...
    def _build_model(self, config):#@# override这个构造自己的verifier
        # the following line is necessary
        ...
        if self.config.model.input_tokenizer is None:
            ...
            input_tokenizer_local_path = copy_to_local(config.model.input_tokenizer)
            self.input_tokenizer = hf_tokenizer(input_tokenizer_local_path,
                                                trust_remote_code=config.model.get('trust_remote_code', False))       #@# actor的tknz
            self.tokenizer = hf_tokenizer(local_path, trust_remote_code=config.model.get('trust_remote_code', False))  #@# RM 的tknz

        trust_remote_code = config.model.get('trust_remote_code', False)
        model_config = AutoConfig.from_pretrained(local_path, trust_remote_code=trust_remote_code)
        model_config.num_labels = 2 #model_config.num_labels = 1 @#最大不同是huatuo verifier的label=2
        ...
        with init_context(), warnings.catch_warnings():
            ...
            reward_module = AutoModelForSequenceClassification.from_pretrained(pretrained_model_name_or_path=local_path,
                                                                            ...
                                                                            torch_dtype=torch.bfloat16,
                                                                            attn_implementation='flash_attention_2',
                                                                            # num_labels=2,  #@#最大不同是huatuo verifier的label=2
                                                                            trust_remote_code=trust_remote_code)
    ...
    VERIFIER_prompt = """<Model Response>
{}
</Model Response>
...
Your task is to evaluate the model response by comparing it to the reference answer. If the model response is correct and aligns with the reference answer, output "True" . If it is incorrect or fails to select the correct option (if options are provided), output "False" . {}"""
    VERIFIER_errholder = "I do not know."
    ThinkL="<think>"
    ThinkR="</think>"
    AnswL="<answer>"
    AnswR="</answer>"
    def _switch_chat_template(self, data: DataProto): #@# override这个定制verifer的问答prompt
        src_max_length = data.batch['attention_mask'].shape[-1]

        src_tokenizer = self.input_tokenizer #@# actor的tknz
        target_tokenizer = self.tokenizer   #@# reward的tknz

        rm_input_ids = []
        rm_attention_mask = []

        for i in range(data.batch.batch_size[0]):  #@#训练循环 这里拿到的batch_size是在trainer中乘过rollout.n的
            # extract raw prompt
            # chat: list = data.non_tensor_batch['raw_prompt'][i].tolist()
            ...
            # remove bos and eos
            response = response.replace(src_tokenizer.eos_token, '')
            #@#参考BatchRewardManager, huatuo verifier还需要拿到ground truth
            ground_truth = data[i].non_tensor_batch['reward_model'].get('ground_truth', None)
            #@#这个response要放入reward/verifier的prompt模板，而且huatuo verif的模板不需要原来的prompt
            # chat.append({'role': 'assistant', 'content': response})
            foundLs = response.split(self.ThinkR)
            foundLsLen = len(foundLs)
            if foundLsLen > 1:
                rspAnsw = foundLs[-1].strip()
                if bool(rspAnsw):
                    rspAnsw = rspAnsw.replace(self.ThinkR,"").replace(self.AnswL,"").replace(self.AnswR,"").strip()
                    rspAnsw = self.VERIFIER_prompt.format(rspAnsw, ground_truth, target_tokenizer.eos_token)
                else:
                    rspAnsw = self.VERIFIER_errholder
            else:
                rspAnsw = self.VERIFIER_errholder
            ...
            #                                                                  add_generation_prompt=False,
            #                                                                  tokenize=False)
            # if self.rank == 0 and i == 0: #@#FORTST
            #     # for debugging purpose
            #     print(f'Switch template. chat: {rspAnsw}')
    ...
    def _forward_micro_batch(self, micro_batch):   #@# override这个定制自己verifier的推理过程，但不知道是否支持rm pad模式
        from flash_attn.bert_padding import pad_input, unpad_input, index_first_axis, rearrange
        from verl.utils.ulysses import ulysses_pad_and_slice_inputs, gather_outpus_and_unpad

        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):#@#TODO trch.inf_mde会否比no_grd好
            input_ids = micro_batch['input_ids']
            batch_size, seqlen = input_ids.shape
            attention_mask = micro_batch['attention_mask']
            position_ids = micro_batch['position_ids']
            if self.use_remove_padding:#@#TODO 自定义 verifier 支持 use_remove_padding
                assert False, "#@# huatuo verif 暂不支持 use_remove_padding"
                input_ids_rmpad, indices, *_ = unpad_input(input_ids.unsqueeze(-1),
                                                           attention_mask)  # input_ids_rmpad (total_nnz, ...)
                input_ids_rmpad = input_ids_rmpad.transpose(0, 1)  # (1, total_nnz)
                ...
                output = self.reward_module(input_ids=input_ids,
                                            ...
                                            position_ids=position_ids,
                                            use_cache=False)
                logits = output.logits  #@#??? 不是(batch_size, seq_len, 1)，是(batchSz, 2)么?
                rm_score = torch.nn.functional.softmax(logits, dim=-1)
            return rm_score[:,1] #@#??? 对于huatuo verif貌似是这样,返回的形状就是(batchSz,)了?
            # rm_score = rm_score.squeeze(-1)
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if use_dynamic_bsz:
                ...
                micro_batches = rm_data.batch.split(self.config.micro_batch_size_per_gpu)  #@#训练循环 这里基于reward_model.micro_batch_size_per_gpu再在原大的batch中切出小batch给reward model计算self._forward_micro_batch()
            output = []
            for micro_batch in micro_batches:
                rm_score = self._forward_micro_batch(micro_batch)
                output.append(rm_score)
            ...
            # token_level_scores = self._expand_to_token_level(data, scores)
            # Note that this is only the scores, may not be the final rewards used to train RL
            output = DataProto.from_dict(tensors={'bt_rm_scores':scores})#@#MODIFIED bt_rm_scores用于reward func拿到后进行加工
            output = self.ulysses_sharding_manager.postprocess_data(data=output)

        # https://pytorch.org/docs/stable/notes/fsdp.html#fsdp-notes
        # unshard the root FSDP module
        if self.world_size > 1:
            self.reward_module._handle.reshard(True) #@#BUG AssertionError: Expects sharded strategy
