# nohup ./runsftBT.sh > runsftBT0327.log 2>&1 &
import os
import json
import torch
import logging
...
class Train_dataset(torch.utils.data.Dataset):
    def __init__(self, config, tokenizer):
        ...
        # self.data = newdata
        self.data = dss.load_from_disk(config.data_path)
    ...
    def get_response(self,da):
        ...
        temp = '<think>\n{}</think>\n\n{}'
        return temp.format(da['reasoning (reasoning_content)'],da['response (content)'])
...
def train(args):
    ...
    if accelerator.is_main_process:
        ...
        tbwriter = SummaryWriter(logdir=args.log_dir)
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for batch_cnt, batch in train_dataloader_iterator:
            ...
            if global_step % 3 == 0 and accelerator.is_main_process and tbwriter is not None:
                tbwriter.add_scalar('loss', train_loss, global_step)
                tbwriter.add_scalar('acc',  acc, global_step)
                tbwriter.add_scalar('lr',  lr_scheduler.get_last_lr()[0], global_step)
                tbwriter.add_scalar('skip', int(accelerator.optimizer_step_was_skipped), global_step)
                # wandb.log({
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_seq_len', default=7088, type=int)
    parser.add_argument('--gradient_checkpointing', action='store_true')
    parser.add_argument('--gradient_accumulation_steps', default=1, type=int)
    parser.add_argument('--train_bsz_per_gpu', default=16, type=int)
