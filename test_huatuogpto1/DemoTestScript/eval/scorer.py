...
def scoreOneItm(txt, options, answer_idx):
    ans,ans_type = match_choice(txt, options)
...
def score(data,ignore_miss= False):
    ...
    for da in data:
        ...
        if ans[0].lower() == da['answer_idx'].lower():
            res[da['source']][1] += 1   #@#输出文本中第一个提及答案选项中正确的数目 head_match_score
            cor_data.append(da)
        else:
            wrong_data.append(da)
        
        if ans[1].lower() == da['answer_idx'].lower():   #@#输出文本中第一个提及答案选项中正确的数目 tail_match_score
            res[da['source']][3] += 1

        res[da['source']][2] += 1  #@#总数
    ...
    cnt = 0
    for k in res:
        ttl = ttl + float(res[k][0])
        cnt = cnt + 1
    res['MEAN'] = float(ttl / cnt)
